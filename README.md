...
- **3D 보기** — `포물선 운동` 부터 RealityKit `RealityView` 로 3D 시점 지원
  (드래그 = 궤도 회전, 핀치 = 줌). 우상단 `2D / 3D` Picker 로 전환. 다른
  실험들도 단계적으로 3D 추가 예정.
- **닫힌 해 우선** — 시뮬마다 ConceptCard 우상단에 **계산 종류 배지** 노출:
  - 🟢 *닫힌 해* — 모든 시간에서 분석해 (대부분 시뮬)
  - 🟣 *이벤트 기반* — 충돌 사이 등속(닫힌 해) + 충돌 시 닫힌 해 임펄스
    (`기체분자운동`, `자유 충돌 박스`)
  - 🟠 *수치* — 다체 문제 등 닫힌 해 부재 영역 (`N체 중력` Velocity-Verlet)
  - **HeatTransfer / Spring / RLC / Lorentz / Kepler / Pendulum** 모두
    적분 없이 닫힌 해 직접 평가 (지수감쇠·감쇠진동·E×B 표류·Kepler 방정식
    Newton 풀이·Jacobi cn 등).
